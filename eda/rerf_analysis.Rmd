...
res.xv <- lapply(1:length(xv.sets), function(i) {
  ...
  forest <- RerF(X.train, Y.train, num.cores=no_cores, cat.map = cat.map)
...
forest <- RerF(X, Y, num.cores=no_cores, cat.map=cat.map, store.impurity=TRUE)
